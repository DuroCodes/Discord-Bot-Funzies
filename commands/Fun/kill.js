...
exports.run = async (bot, message, args, functions) => {
  ...
  const kill = [
    ...
    "$mention shat pants too hard.",
    "$mention fucked $author's sibling too many times and dad found out so he shot them",
    "$author fucked $mention's mom too hard"
  ];

  if(!message.mentions.members.first()) return functions.errorEmbed(message, message.channel, 'Please input a user you wish to kill.');

  var killMsg = kill[Math.floor(Math.random() * kill.length)]
    .replace(/\$mention/g, message.mentions.members.first().displayName)
